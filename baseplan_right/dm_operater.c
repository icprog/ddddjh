...
int write_act_info(int result,char *note)
{
	...
	sprintf(sql_string,"insert into LJJNDB.LJJNDB.T_CXZXJG "
		"(ID,CXID,CXMC,ZQCWXX,ZQCWXXMS,CXZXRQ,CXZXSJ)"
		"values('%s','%s','%s','%d','%s','%s','%s')"
		,act_info.id,act_info.prog_id,
		act_info.prog_name,act_info.result,
		act_info.result_desc,act_info.date,
		act_info.time);
	printf("%s\n",sql_string);
...
/*
*函数说明：用于执行达梦数据库sql语句,函数内部实现语句执行、列绑定、记录数返回、fetch操作
*输入参数：sql_string所要执行的sql语句，column_num select语句返回的列的数目
*		   desc对select语句所取数据的描述
*输出参数：db_data select语句执行结果存放在此结构中
*返回值：  返回select语句返回记录的数目
*/
...
//============================================获取电厂出力上下限=======================================
	...
	*db_data = malloc( column_num*row_num*COLUMN_LEN );
	...
	/*	printf("cccc%s,%s\n",
				*db_data+i*column_num*COLUMN_LEN + 0*COLUMN_LEN,
				*db_data+i*column_num*COLUMN_LEN + 1*COLUMN_LEN);*/
