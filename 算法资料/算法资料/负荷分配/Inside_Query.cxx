...
//	CS_COMMAND       *cmd_ptr=NULL;          /* handle for this SQL command batch*/
	...
	str_h=sq_str;
	str_t=sq_str;
	while(strncmp(str_h,COL_END_FIX,COL_END_FIX_LEN)) 
		str_h++;
	*str_h='\0';
	strcpy(sysid,str_t);
		...
		switch(result_type)
		{
			...
			//	printf("col_num=%d\n",col_num);
						...
						&rows_read)==CS_SUCCEED)
				{
						...
						{
							buf[j][datalen[j]]='\0';
							strcpy(&buf_str[count*MAX_COLUMN_NUM*(MAX_COLUMN_SIZE+1)+j*(MAX_COLUMN_SIZE+1)],buf[j]);
						}
					count++;
				}
